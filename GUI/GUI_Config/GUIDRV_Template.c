...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
    ...
    GUI_USE_PARA(PixelIndex);
    {
      ...
      // TBD by customer...
      //
			//DrawPixel(x,y,PixelIndex);			//¿ìËÙ´òµãº¯Êý
		...
		LCD_WR_Comd(0x002c);   		//´æ´¢Æ÷Ð´
		//BlockWrite(x,x+1,y,y+1);
		LCD_WR_DATA(PixelIndex>>8);
...
/*
  ...
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    ...
    for (; y0 <= y1; y0++) {
      for (x = x0; x <= x1; x++) {
       // _SetPixelIndex(pDevice, x, y0, PixelIndex);
				Lcd_ColorBox(x0,x1,y0,y1,PixelIndex);		//Ìî³äº¯Êý
      }
    }
  }*/
	Lcd_ColorBox(x0,x1,y0,y1,LCD_COLORINDEX);		//Ìî³äº¯Êý
	/*
	LCD_WR_Comd(0x2a);   
	LCD_WR_DATA(x0>>8);
	LCD_WR_DATA(x0);
	...
	for(temp=((x1-x0)+1)*((y1-y0)+1); temp>0; temp--)
		{
			LCD_WR_DATA(LCD_COLORINDEX>>8);
